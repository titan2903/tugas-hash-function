...
// Fungsi hash: Division Method untuk int
int divisionHashFunction(int key, int tableSize)
{
    return key % tableSize;
}

// Fungsi hash: Division Method untuk string
int stringDivisionHashFunction(const string &key, int tableSize)
{
    long long sum = 0;
    for (char c : key)
    {
...
{
    cout << "\nRunning test for dataset " << datasetNum << " and table size " << tableSize << ", using Division Method" << endl;
...
int main()
{
    ...
    // Memanggil fungsi untuk metode division untuk setiap dataset dan ukuran tabel
    for (int tableSize : {BUCKET_SIZE_SMALL, BUCKET_SIZE_MEDIUM, BUCKET_SIZE_LARGE}) // Loop untuk setiap ukuran tabel
    {
        runHashTest(dataset1, tableSize, divisionHashFunction, 1);
        runHashTest(dataset2, tableSize, divisionHashFunction, 2);
        runHashTest(dataset3, tableSize, divisionHashFunction, 3);
        runHashTestStr(dataset4, tableSize, stringDivisionHashFunction, 4);
    }
