...
// Fungsi hash: Folding Method untuk int
int foldingHashFunction(int key, int tableSize)
{
    int sum = 0;
    while (key > 0)
    {
        sum += key % 1000; // Ambil 3 digit terakhir
        key /= 1000;       // Hapus 3 digit terakhir
    }
    return sum % tableSize;
}

// Fungsi hash: Folding Method untuk string
int stringFoldingHashFunction(const string &key, int tableSize)
{
    long long sum = 0;
    int partSum = 0;
    int count = 0;
    ...
    sum += partSum; // Menambahkan sisa yang belum dihitung
    return sum % tableSize;
}
...
{
    cout << "\nRunning test for dataset " << datasetNum << " and table size " << tableSize << ", using Folding Method" << endl;
...
int main()
{
    ...
    // Memanggil fungsi untuk metode folding untuk setiap dataset dan ukuran tabel
    for (int tableSize : {BUCKET_SIZE_SMALL, BUCKET_SIZE_MEDIUM, BUCKET_SIZE_LARGE}) // Loop untuk setiap ukuran tabel
    {
        runHashTest(dataset1, tableSize, foldingHashFunction, 1);
        runHashTest(dataset2, tableSize, foldingHashFunction, 2);
        runHashTest(dataset3, tableSize, foldingHashFunction, 3);
        runHashTestStr(dataset4, tableSize, stringFoldingHashFunction, 4);
    }
